...
sub generate_overview {
    ...
    else
    {
        ...
        if (-e '/home/lacuna/server/var/www/public/server_overview.json')
        {
            open my $read, '<', '/home/lacuna/server/var/www/public/server_overview.json';
            my $old_stats = from_json(do { local $/; <$read> });
            $out{spies} = $old_stats->{spies};
        }

        open my $fh, '>', '/home/lacuna/server/var/www/public/server_overview.json';
...
#  close(OUT);
  ...
  if ($config->get('access_key')) {
    ...
    {
        open my $fh, '>', '/home/lacuna/server/var/www/public/starmap.json';
        print $fh $json_txt;
        close $fh;
    }
}
